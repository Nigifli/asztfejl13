...
public class MotorcycleModelValidator: BaseValidator<MotorcycleModel>
{
    ...
    public MotorcycleModelValidator(IHttpContextAccessor httpContextAccessor) : base(httpContextAccessor)
    {
        if (IsPutMethod)
        {
            RuleFor(x => x.Id).NotEmpty().WithMessage("Id is required"); // hazi: validalni h id letezik az adatbazisban
        }
...
;       RuleFor(x => x.Model).NotEmpty().WithMessage("Model is required");
        ...
        RuleFor(x => x.Manufacturer).NotNull().WithMessage("Manufacturer is required"); // hazi: validalni h manufacturer id letezik az adatbazisban
        RuleFor(x => x.Manufacturer.Id).GreaterThan(0).WithMessage("Manufacturer ID has to be greater than 0"); 

        RuleFor(x => x.Type).NotNull().WithMessage("Type is required"); // hazi: validalni h type id letezik az adatbazisban
